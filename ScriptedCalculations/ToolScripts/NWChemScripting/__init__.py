...
# Returns number of highest xyz_file
...
def start_job(jobfile='job.run'):
	return start_job_mpi(jobfile)

# Queue a job and return error code of queueing call
def start_job_mpi(jobfile='job.run'):
	return run(['sbatch'], jobfile).returncode

# Run an nwchem job and return result
def run_nwchem_job(jobfile='input.nw', cores=16):
	return run(['mpirun', 'nwchem', '-n {}'.format(cores), jobfile], capture_output=True).stdout
