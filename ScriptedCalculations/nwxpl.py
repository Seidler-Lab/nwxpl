...
def run_calculation(structfilename, env_config, basisfilename, workdir, scratchdir, outdir, atom, charge):
    ...
    #setup_esp_filestructure(structfilename, env_config, basisfilename, workdir,
                             scratchdir, outdir, atom, charge=charge)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--atom', action='store', dest='atom', type=str, default='Ca',
        help="Specify element, or atom, of which to perform calculations")
    parser.add_argument('-c', '--charge', action='store', dest='charge', type=float, default=0,
    ...
    for structfilename in structurelist:
        run_calculation(structfilename, ENV_CONFIG, BASIS_FILENAME, WORK_DIR,
                        SCRATCH_DIR, OUT_DIR, ATOM, CHARGE)
